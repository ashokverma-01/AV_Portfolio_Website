...
export const login = (email, password) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.post(
      ...
      { email, password }, // Send email & password in the request body
      { withCredentials: true, headers: { "Content-Type": "application/json" } } // Corrected placement of config
    );
...
export const getUser = () => async (dispatch) => {
  ...
  try {
    const { data } = await axios.get("http://localhost:5200/api/v1/user/me", {
  ...
  (currentPassword, newPassword, confirmNewPassword) => async (dispatch) => {
    ...
    try {
      const { data } = await axios.put(
        "http://localhost:5200/api/v1/user/password/update",
...
export const updateProfile = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(
      "http://localhost:5200/api/v1/user/me/profile/update",
      data,
      {
        withCredentials: true,
