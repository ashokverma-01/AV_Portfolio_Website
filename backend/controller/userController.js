...
export const register = catchAsyncErrors(async (req, res, next) => {
  ...
  const { fullName, email, password, phone, aboutMe, portfolioURL } = req.body;

  // Check if required fields are missing
  if (!phone || !aboutMe || !portfolioURL) {
    return next(
      new ErrorHandler("Portfolio URL, About Me, and Phone are required!", 400)
    );
  }
...
export const logout = catchAsyncErrors(async (req, res, next) => {
  res
    ...
    .cookie("token", "", {
      httpOnly: true,
      expires: new Date(Date.now() + 1000), // ✅ Expires 1 second after logout
    })
    .json({
      success: true,
      message: "Logged Out!",
...
export const getUserForPortfolio = catchAsyncErrors(async (req, res, next) => {
  const id = "67ee6c5df4c4ce80beca8c1f";
